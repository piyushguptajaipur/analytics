...
setwd("~/Google Drive/GreatLakes Capstone Project/Retailers/01Gifting/Analysis/MBA/")

#Import Data in R
InvoiceDetails1 <- read.csv(file.choose(),header = T)
InvoiceDetails2 <- read.csv(file.choose(),header = T)

InvoiceDetails <- rbind(InvoiceDetails1,InvoiceDetails2)
...
#Drop 'invalid' rows
ProductServices <- ProductServices[!ProductServices$NewPdtName == "invalid",]
...
#Table Merged
Table_M <- NULL
Table_M$TransactionID <- Merged$InvoiceID
Table_M$Item <- Merged$NewPdtName
...
#Extended rules
ext_rules <- apriori(txn, parameter = list(supp = 0.00001, conf = 0.1,minlen=3))
length(ext_rules)
...
# Get the rules
basket_rules <- apriori(txn, parameter = list(supp = 0.0001, conf = 0.8,minlen=3))
length(basket_rules)
